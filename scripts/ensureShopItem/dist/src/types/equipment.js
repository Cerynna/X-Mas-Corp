...
const calculatePrice = (level, quality) => {
    return Math.floor(level * qualityPriceMultiplier[quality] * 100);
};
const generatorNameItem = (slot, quality, classInfo) => {
    ...
    if (slot === "weapon" && classInfo && classInfo.weaponType) {
        ...
        const possibleWeapons = classInfo.weaponType.filter((wt) => weaponNames[wt] && weaponNames[wt][quality]);
        const chosenWeaponType = possibleWeapons[Math.floor(Math.random() * possibleWeapons.length)];
        name =
            weaponNames[chosenWeaponType][quality][Math.floor(Math.random() * weaponNames[chosenWeaponType][quality].length)];
    }
    else if (slot === "jewelry") {
        name = `${nameOptions[Math.floor(Math.random() * nameOptions.length)]}`;
    }
    else {
        name = `${nameOptions[Math.floor(Math.random() * nameOptions.length)]} ${itemTypesText[classInfo ? classInfo.armorType : "cloth"]}`;
    }
    return name;
};
const calculateQuality = (level, qualityRoll) => {
    let quality = "poor";
    if (level < 10) {
        if (qualityRoll < 0.5)
            quality = "poor";
...
const calculateEquipmentStats = (slot, classInfo, monsterLevel, quality, luck) => {
    const stats = {};
    const baseStatValue = Math.floor(monsterLevel * 0.8 * qualityMultiplier[quality]);
    ...
    if (slot === "weapon") {
        ...
        // Crit chance proc
        if (Math.random() < 0.25) {
            stats.critChance = Math.floor(baseStatValue * (0.1 + Math.random() * 0.2));
        }
    }
    else if (slot === "jewelry") {
        ...
        if (Math.random() > critChanceRoll) {
            stats.critChance = Math.floor(baseStatValue * (0.08 + Math.random() * 0.15));
        }
    }
    return stats;
};
